...
export const getPersonalizedTemplates = async (req,res)=>{
	...
	const { success, error, personalizedTemplates ,userResumeId } = await personalizedTemplatesHandler(req.body,userId);
    
	if (!success) {
		return missingFieldsError(res, error);
	}

	const {success:userProfileSuccess,user,error:userProfileError} = await getUserProfileHandler(userId);

	if (!userProfileSuccess) {
		return devError(res, userProfileError);
	}
