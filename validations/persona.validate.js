...
const personaValidation = Joi.object({
	...
	background: Joi.object({
		yearsOfExperience: Joi.number().integer().positive().required(),
		education: Joi.object({
			level: Joi.string().valid("graduated", "inCollege", "postGraduate").required()
		}),
		hasProjects: Joi.boolean().required(),
		hasCertifications: Joi.boolean().required(),
